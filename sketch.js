...
var gameState=PLAY;
var distance=0
var input,form,title,buttun,car1,car,rty,shield,slider,hypeDocument,nameName,overbug,buttun1,input1;
var score=0
var SELECTOR=2
var sheildTime=5
var coin=0
...
var MENU=12
var SETTINGS=13
var carS1,carS2,carS3,carS4,playerCar1,acc,con,upcon,downcon;
var FIRSTBG=9
var PLAYERCAR1=7
var BUGATTIGAME=8
var Gear;
...
accImg=loadImage("accelerate.png")
conImg=loadImage("controller.png")
}
function setup(){
 ...
 playerCar1.visible=false
  ...
  acc=createSprite(700,450,50,50)
  acc.addImage(accImg)
  acc.scale=0.6
  acc.visible=false

  con=createSprite(100,430,20,20)
  con.addImage(conImg);
  con.scale=0.8
  con.visible=false

  upcon=createSprite(100,390,40,40)
  upcon.visible=false;
  downcon=createSprite(100,470,40,40)
  downcon.visible=false

 console.log(gameState)
 upS=createSprite(0,0,1000,10)
 downS=createSprite(50,500,1000,10)
upS.visible=false;
downS.visible=false;
  ...
  shieldGR=new Group();
  carGR=new Group();
  carGR.add(car,playerCar1,BugattiPla)
  if(carGR.y===500||carGR.y===0){
    carGR.y=carGR.y
  }
...
strokeWeight(4)
  ...
  moreG.visible=false;
  if(frameCount%10===0){
  if(mouseIsOver(moreG)){
    open("https://abdulwahab321.github.io/Moregames/")
  }
}
}
if(gameState!==START&&gameState!==SELECTOR&&gameState!==END&&gameState!==GARAGE&&gameState!==INS&&gameState!==FIRSTBG&&gameState!==CARSELECTOR){
...
if(gameState===PLAY){
  ...
  if(mouseIsOver(upcon)){
    car.velocityY=-5
  }
  if(mouseIsOver(downcon)){
    car.velocityY=5
  }
...
if(gameState===GARAGE){
  ...
  if(frameCount%5===0){
  if(mouseIsOver(sheildValueIncreaser)){
...
if(gameState===START){
  ...
  how.visible=false;
  text("Type your Name Here",300,140);
  input.position(330,160)
  buttun.position(450,200)
...
if(gameState===PLAYERCAR1){
  ...
  invaderGr.collide(invaderGr);
  invaderGr.collide(invaderGr);
  playerCar1.collide(upS)
  playerCar1.collide(downS)
  updateDistance();
  if(keyWentDown("g")){
    gameState=GARAGE
  }
  ...
  if(playerCar1.y===500||playerCar1.y===0){
    playerCar1.y=playerCar1.y
  }
  ...
  if(gameState===BUGATTIGAME){
    ...
    invaderGr.collide(invaderGr);
    invaderGr.collide(invaderGr);
    BugattiPla.collide(upS)
    BugattiPla.collide(downS)
    updateDistance();
    if(keyWentDown("g")){
      gameState=GARAGE
    }
    ...
    if(BugattiPla.y===500||BugattiPla.y===0){
      BugattiPla.y=BugattiPla.y
    }
...
function spawnObstacles(){
 ...
 function text(){
  ...
  function reset(){
    if(keyDown("space")||keyDown("r")||mouseIsOver(over)){
      gameState=SELECTOR
      score=000
      car.scale=0.6
      sheildTime=5
...
function say(){
  ...
  sp.text="Hai Welcome To street Racer mobile edition"
